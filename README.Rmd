...
If interest is in model estimation but not prediction, then use `SEIRfansy()`. Otherwise, use `predict()` (see below).
...
If interest is in model estimation and prediction, then use `predict()`, which first runs `SEIRfansy()` internally, and then predicts.
